        ...
        for station in stations {
            alertController.addAction(UIAlertAction(title: "\(station.name!)",style: .default, handler: {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                    self.updatePosition(player: self.player!) //recursion
                }
            }))
        }
...
//        }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        serialQueue.sync {
            self.updatePosition(player: self.player!)
        }
    }
    ...
    func updatePosition(player: Player){
        ...
        if player.remainSteps > 0 {
            ...
            if adjStation.count == 1 {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                    self.updatePosition(player: player) //recursion
                }
